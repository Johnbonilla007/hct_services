    ...
    db.sequelize
      ...
      .then(() => {
        ...
        getProducts().then(productsToSave => {
          db.Product.bulkCreate(productsToSave, {
            include: [db.ProductCategory, db.ProductCity],
          });
        });
...
const getProducts = async () => {
  const categoryProducts = products.reduce(
    (acc, current) => [...acc, ...current.category],
    [],
  );
  ...
  const cityProducts = products.reduce(
    (acc, current) => [...acc, ...Object.keys(current.city || [])],
    [],
  );
  ...
  return products.map(product => ({
    ...
    ProductCategories: product.category
      ? categories
          .filter(s => product.category.includes(s.name))
          .map(cat => ({ categoryId: cat.id }))
      : [],

    ProductCities: product.city
      ? cities
          .filter(s => product.city[s.name])
          .map(c => ({
            cityId: c.id,
