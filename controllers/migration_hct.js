...
const applyMigrationAsync = async (req, res, next) => {
  ...
  const promotionResponse = await PromotionService.addPromotionsAsync(
    promotions,
  );
  ...
  res.json({
    message: {
      category: {
        message:
          categoryResponse.code === httpCode.OK
            ? 'Ok'
            : categoryResponse.message,
      },
      carrusel: {
        message:
          carruselResponse.code === httpCode.OK
            ? 'Ok'
            : carruselResponse.message,
      },
      promotion: {
        message:
          promotionResponse.code === httpCode.OK
            ? 'Ok'
            : promotionResponse.message,
      },
      product: {
        message:
          productResponse.code === httpCode.OK ? 'Ok' : productResponse.message,
      },
    },
  });
};
