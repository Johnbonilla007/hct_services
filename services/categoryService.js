...
const { isValidArray, isValidObj, unrealized, distinctBy } = require("../commons/utilities");
...
class CategoryService {
    ...
    static addCategoriesAsync = async (categories) => {
        if (!isValidArray(categories))
            return { code: httpCode.BAD_REQUEST, message: messages.fieldCategories_required };
        ...
        if (isValidArray(categoriesFound)) {
            ...
            const newSubCategoriesToExistCategories = categories.reduce((acc, category) => {
                if (!isValidArray(category.subCategories))
                    return acc;


                const categoryFound = categoriesFound.find(s => s.name.includes(category.name));

                let newSubCategories = [];
                if (isValidArray(categoryFound.subCategories)) { // Comparar las subCategorías y obtener las que no existen.
                    newSubCategories = category.subCategories.filter(subCategory => {
                        if (categoryFound.subCategories.some(s => s.name.includes(subCategory.name)))
                            return false;
                ...
                newSubCategories = isValidArray(newSubCategories) ? newSubCategories : categoryFound.subCategories;

                // Mapear las subCategorias que no existen.
                return [...acc, ...(newSubCategories.map(subCategory => ({ categoryId: categoryFound.id, ...subCategory })))]
            }, []);

            // Guardar las subCategorias a las categorias existentes.
            await SubCategory.bulkCreate(newSubCategoriesToExistCategories);

            return { code: httpCode.OK, message: messages.createdSuccess("La categoría") };
        }
    ...
    static addCategoryAsync = async ({ name, active, subCategories }) => {
        ...
        if (!name)
            return ({ code: httpCode.BAD_REQUEST, message: messages.fieldUserRequired });

        const category = await Category.findOne({ where: { name } });

        // Sí existe la categoría se procederán a agregar las subCategorías.
        if (isValidObj(category)) {
            return await CategoryService.addSubCategoriesToCategoyIdAsync(category.id, subCategories);
        }
    ...
    static addSubCategoriesToCategoyIdAsync = async (categoryId, subCategories) => {

        if (!categoryId || categoryId <= 0 || !isValidArray(subCategories))
            return { code: httpCode.BAD_REQUEST, message: messages.fieldSubCategory_required };
        ...
        const subCategoriesToCreate = subCategories.filter(s => isValidArray(subCategoriesAssigned) ?
            !subCategoriesAssigned.some(sub => sub.name?.includes(s.name)) : true)
            .map(subCategory => ({
                categoryId,
        ...
        if (!isValidArray(subCategoriesToCreate))
            return { code: httpCode.OK, title: info.ADVERTENCIA, message: messages.subCategoriesAlready };
